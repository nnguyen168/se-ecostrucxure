...
# Process app.yaml to substitute environment variables
if [ -f "app.yaml" ] && [ -f ".env.local" ]; then
  echo "🔧 Processing app.yaml with environment variables..."

  # Source the .env.local file to get environment variables
  source .env.local

  # Create a temporary app.yaml with substituted values
  cp app.yaml app.yaml.tmp

  # Substitute environment variables if they exist
  if [ -n "$DATABRICKS_GENIE_SPACE_ID" ]; then
    sed -i.bak "s|\${DATABRICKS_GENIE_SPACE_ID}|$DATABRICKS_GENIE_SPACE_ID|g" app.yaml.tmp
  fi

  if [ -n "$DATABRICKS_TOKEN" ]; then
    sed -i.bak "s|\${DATABRICKS_TOKEN}|$DATABRICKS_TOKEN|g" app.yaml.tmp
  fi

  if [ -n "$DATABRICKS_HOST" ]; then
    sed -i.bak "s|\${DATABRICKS_HOST}|$DATABRICKS_HOST|g" app.yaml.tmp
  fi

  # Replace the original app.yaml temporarily
  mv app.yaml app.yaml.original
  mv app.yaml.tmp app.yaml
  rm -f app.yaml.tmp.bak
fi
...
# Restore original app.yaml if it was processed
if [ -f "app.yaml.original" ]; then
  mv app.yaml.original app.yaml
fi
